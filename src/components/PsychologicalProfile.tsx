...
export function PsychologicalProfile({ videoAnalysis, audioAnalysis }: PsychologicalProfileProps) {
  ...
  const buildBasicPsychProfile = (history: any[], video: VideoAnalysis | null, audio: AudioAnalysis | null) => {
    ...
    return {
      personality: {
        bigFive: {
          openness: Math.random() * 0.3 + 0.5,
          conscientiousness: behavioralData.consistency,
          extraversion: communicationData.expressiveness,
      ...
      emotionalIntelligence: {
        ...
        selfRegulation: emotionalData.regulation,
        motivation: 0.7,
        empathy: emotionalData.empathy,
        socialSkills: communicationData.effectiveness,
        strengths: emotionalData.awareness > 0.7 ? ['Strong emotional awareness'] : ['Developing emotional skills'],
        developmentOpportunities: emotionalData.regulation < 0.6 ? ['Emotional regulation'] : ['Advanced emotional skills']
      },
      cognitiveBiases: {
        confirmationBias: Math.random() * 0.4 + 0.2,
        catastrophicThinking: emotionalData.stress * 0.8,
        allOrNothingThinking: Math.random() * 0.3 + 0.1,
        personalizedInsights: ['Developing pattern recognition skills']
      },
      attachmentStyle: {
        primaryStyle: emotionalData.empathy > 0.7 ? 'secure' : 'developing',
        securityLevel: emotionalData.empathy,
        relationshipPatterns: ['Building healthy communication'],
        growthRecommendations: ['Continue developing emotional awareness']
      },
      decisionMakingStyle: {
        style: behavioralData.thoroughness > 0.6 ? 'analytical' : 'intuitive',
        confidence: behavioralData.confidence,
        riskTolerance: 0.5,
        decisionTriggers: ['clear information', 'time pressure'],
        paralysisPoints: behavioralData.thoroughness > 0.8 ? ['too many options'] : []
      },
      stressResponse: {
        primaryResponse: emotionalData.stress > 0.6 ? 'freeze' : 'problem-solve',
        burnoutRisk: emotionalData.stress * 0.7,
        copingMechanisms: ['reflection', 'seeking support'],
        resilienceFactors: ['self-awareness', 'help-seeking']
      }
    };
  };

  const analyzeEmotionalPatterns = (history: any[], video: VideoAnalysis | null, audio: AudioAnalysis | null) => {
    const emotionalWords = history.flatMap(msg => 
      (msg.content || '').toLowerCase().match(/\b(happy|sad|angry|worried|excited|frustrated|peaceful|anxious)\b/g) || []
    );
    
    const stressWords = history.flatMap(msg =>
      (msg.content || '').toLowerCase().match(/\b(stress|overwhelm|pressure|burden|difficult)\b/g) || []
    );
    
    return {
      awareness: video?.attentiveness || 0.6,
      regulation: audio?.clarity || 0.6,
      empathy: emotionalWords.length > 3 ? 0.8 : 0.6,
      stress: Math.min(1, stressWords.length * 0.2)
    };
  };

  const analyzeCommunicationPatterns = (history: any[], audio: AudioAnalysis | null) => {
    const avgMessageLength = history.reduce((acc, msg) => acc + (msg.content?.length || 0), 0) / (history.length || 1);
    
    return {
      expressiveness: avgMessageLength > 100 ? 0.8 : 0.5,
      effectiveness: audio?.clarity || 0.6
    };
  };

  const analyzeBehavioralPatterns = (history: any[], video: VideoAnalysis | null) => {
    const questionCount = history.filter(msg => msg.content?.includes('?')).length;
    const thoughtfulWords = history.flatMap(msg =>
      (msg.content || '').toLowerCase().match(/\b(think|consider|reflect|analyze|understand)\b/g) || []
    );
    
    return {
      consistency: video?.attentiveness || 0.7,
      thoroughness: thoughtfulWords.length > 5 ? 0.8 : 0.5,
      confidence: questionCount < history.length * 0.2 ? 0.8 : 0.6
    };
  };

  const identifyStrengths = (emotional: any, communication: any, behavioral: any) => {
    const strengths = [];
    if (emotional.awareness > 0.7) strengths.push('Self-aware and reflective');
    if (communication.expressiveness > 0.7) strengths.push('Articulate communicator');
    if (behavioral.thoroughness > 0.7) strengths.push('Thoughtful and analytical');
    if (emotional.empathy > 0.7) strengths.push('Empathetic and caring');
    return strengths.length > 0 ? strengths : ['Developing personal strengths'];
  };

  const identifyMotivations = (history: any[]) => {
    const allText = history.map(msg => msg.content || '').join(' ').toLowerCase();
    const motivations = [];
    
    if (allText.includes('grow') || allText.includes('improve')) motivations.push('Personal development');
    if (allText.includes('help') || allText.includes('support')) motivations.push('Helping others');
    if (allText.includes('family') || allText.includes('relationship')) motivations.push('Meaningful relationships');
    if (allText.includes('success') || allText.includes('achieve')) motivations.push('Achievement');
    
    return motivations.length > 0 ? motivations : ['Understanding and growth'];
  };
